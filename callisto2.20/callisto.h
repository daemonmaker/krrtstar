...
typedef void (*CAL_UserDrawnCallback)(void *);
...
/*! Loads a texture to memory. The texture must be in the .ppm or the .png format.
...
/*! Creates an object group.
  ...
  \param visible Set to false if group should not be visble.
	\returns The function returns #CAL_SUCCESS on success, and an \ref Errors "errorcode" on failure.
  \ingroup Group
*/
int CAL_CreateGroup (int* groupID, int parentID, bool colCheck, char* label="", bool collapsed=false, bool visible=true);
...
/*! Adds a keystate for the group animation.
  ...
  \param visible If set the main view group visibility adapts to this parameter.
	\returns The function returns #CAL_SUCCESS on success, and an \ref Errors "errorcode" on failure.
  \ingroup Animation
*/
int CAL_AddGroupKeyState (int groupID, float time, CAL_scalar *position, CAL_scalar *orientation=CAL_NULL, CAL_scalar *scaling=CAL_NULL, bool isQuat=true, bool visible=true);
...
/*! Set a callback function, this is called when the user selects pressed a key that is NOT reserved in Callisto.
...
/*! Set a callback function, this is called when the user selects an object by using the mouse and pressing SPACE.
  ...
  \ingroup AdvancedObject
*/
int CAL_SetObjectSelectCallback (CAL_ObjectSelectCallback cb, void* data = NULL);
...
/*! Create a triangle mesh.
  ...
  \param *p List of coordinates of the object. The size of the list must be 3*nt.
  \param objID Set to the object ID.
  \param label Label of the object. Default is no label.
	\returns The function returns #CAL_SUCCESS on success, and an \ref Errors "errorcode" on failure.
  \ingroup Objectdef
...
/*! Create a user drawn object. This object is only useable in the visualisation, and not for collision detection.
  \param groupID The group ID to put the object in.
  \param callback The user drawn callback.
  \param x The x position of the object.
  \param y The y position of the object.
  \param z The z position of the object.
  \param objID Set to the object ID.
  ...
  \ingroup AdvancedObject
*/
int CAL_CreateUserDrawn (int groupID, CAL_UserDrawnCallback callback, CAL_scalar x, CAL_scalar y, CAL_scalar z, int* objID=CAL_NULL, char* label="");
